...
export const PinContainer = ({ children, title, href, className, containerClassName }) => {
    ...
    return (
        <Link
            className={cn("relative group/pin z-0 cursor-none", containerClassName)}
            ...
            <motion.div className="pointer-events-none w-96 h-80 flex items-center justify-center opacity-0 group-hover/pin:opacity-100 z-[60] transition duration-500">
                <div className="w-full h-full -mt-7 flex-none inset-0">
                    <div className="absolute top-0 inset-x-0 flex justify-center">
                        <div className="relative flex space-x-2 items-center z-10 rounded-full py-0.5 px-4 ring-1 ring-white/30">
                            <span className="relative z-20 text-white text-sm font-bold inline-block py-0.5">{title}</span>
                    ...
                    <>
                        <motion.div className="absolute right-1/2 bottom-1/2 bg-gradient-to-b from-transparent to-cyan-500 translate-y-[14px] w-[2px] h-20 group-hover/pin:h-60 blur-[2px]" />
